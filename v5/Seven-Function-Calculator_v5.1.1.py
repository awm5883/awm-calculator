...
goAgainVar = True
firstTime = True
def goAgain(firstTime): # Perform another calculation message and surrounding computation.
    if firstTime == True:
        firstTime = False
        return 2
    else:
        ...
        if goAgainStr == 'y':
            ...
            return 1
def mainMenu(): # Print main menu and recieve operation 
    print("\n\n\n\n\n\n\n\n\n\n\n")
...
def trigMenu(): # Trig menu (also returns input)
    ...
    return command
def main(operation):
    if operation != '+' and operation != '*' and operation != '-' and operation != '/' and operation != '^' and operation != 't' and operation != 'r': # Invalid operation check
    ...
    if operation == 't': # Trig
        ...
        trigCmdStr = None 
        try:
            if trigCommand[0] == 'a': # Determine command and perform calculation
                if trigCommand[1] == 's':
                    print("arcsin(___) ")
                    print("       ^^^  ")
                ...
                return 1
            try:
                print(f"The {trigCmdStr} of {trigValue} degrees is {answer:.5f}") # Print answer
                return 0
            except:
        ...
        print(f"The {num1}th root of {num2} is {str(answer)}")
        return 0
    print("____ " + operation + " ____") # All other computations
    print("^^^^       ")
    print("ENTER VALUE HERE")
    ...
    if operation == '*': # Calculations for each operation and answer printing.
        answer = int(num1) * int(num2)
        print(f"The product of {str(num1)} and {str(num2)} is {str(answer)}")
        return 0
    elif operation == '+':
        answer = int(num1) + int(num2)
        print(f"The sum of {str(num1)} and {str(num2)} is {str(answer)}")
        return 0
    elif operation == '-':
        answer = int(num1) - int(num2)
        print(f"The difference between {str(num1)} and {str(num2)} is {str(answer)}")
        return 0
    elif operation == '/':
        try:
            answer = int(num1) / int(num2)
        ...
        if answer.is_integer():
            ans_int = int(answer)
        print(f"The quotient between {str(num1)} and {str(num2)} is {str(ans_int)}")
        return 0
    elif operation == '^':
        answer = int(num1) ** int(num2)
        try:
            print(f"{str(num1)} to the power of {str(num2)} equals {str(answer)}")
        except:
