...
def main(operation): # Main! Now we're talking! Most of the "meat" of the code is right here, so strap in!
        ...
        try: 
            if trigCommand[0] == 'a': # BIG elif stack incoming!
                ...
                if trigCommand[1] == 's': 
                    ...
                    return 1
            elif trigCommand == 's': # Now, we check the WHOLE string to see if it's 's'. This is because we're not handling inverse functions anymore and the whole command should be one character.
                print("sin(___) ")
                print("    ^^^  ")
                print("SELECT VALUE (IN DEGREES)\n") # Specify units!
                trigValue = input().strip().lower() # More suffixes
                answer = math.sin(math.radians(float(trigValue)))
                trigCmdStr = "sine" # TrigCmdStr is the STRING that has the command (i.e. sine, cosine, tangent.)
            elif trigCommand == 'c': # Again, now checking 'c' for cosine.
                print("cos(___) ")
                print("    ^^^  ")
                print("SELECT VALUE (IN DEGREES)\n")
                trigValue = input().strip().lower()
                answer = math.cos(math.radians(float(trigValue))) # More "type manipulation." Be careful and watch for errors with these kind of operations!
                trigCmdStr = "cosine"
            elif trigCommand == 't':
                print("tan(___) ")
                print("    ^^^  ")
                ...
                print("ERROR: Invalid operation in function trig") # If trigCommand is none of the valid options.
                return 1
            try: # Another try:, this one nested!
                print(f"The {trigCmdStr} of {trigValue} degrees is {answer:.5f}") # Print the answer. f"Whatever {variable} string" is called an f-string and they can be used instead of adding strings for when you need variables and strings to print on one line.
                return 0 # Success!
            except:
                print("ERROR: Error printing answer in function trig")
                return 1
        except: # The except: of your first try:!
            print("ERROR: Computation error in function trig")
            return 1
    elif operation == 'r': # Second operation, and it gets easier from here.
        print("____th root of ____")
        print("^^^^               ") # More "art" to show the value being entered.
        print("ENTER VALUE        ")
        num1 = input() # Recieve the first number and store it.
        print(num1.zfill(2) + "th root of ____") # Adding strings, and the variable num1 has the modifier .zfill(2). This means if the number is less than (2) digits long, the function adds leading zeroes.
        print("            ^^^^")
        print("ENTER VALUE HERE")
        num2 = input()
        answer = float(num2) ** (1 / float(num1)) # Performing Square Root
        print("The " + num1 + "th root of " + num2 + " is " + str(answer))
        return 0
    print("____ " + operation + " ____") # This next section of code is used for all other operations. (+, -, *, /, ^) Notice the addition of strings.
    print("^^^^       ")
    print("ENTER VALUE")
    num1 = input("1st Value: ")
    try:
        print(num1.zfill(4) + " " + operation + " ____") # This .zfill() function makes the number 4 characters long!
    except:
    ...
    if operation == '*': # Now, we get specific with the operation because we're performing the calculations.
        answer = int(num1) * int(num2) # Multiplication
        try:
            print("The product of " + str(num1) + " and " + str(num2) + " is " + str(answer)) # Notice the string data types
        except:
            print("ERROR: Answer print error in function main")
            return 1
        return 0 # Success!
    elif operation == '+': # Try to figure this one (and minus) out on your own!
        ...
        try:
            ...
            print("ERROR: Cannot divide by zero!") # Division needs this statement, but the others don't.
            return 1
        try:
        ...
        print("ERROR: Invalid operation in function main")
while True: # This runs forever, or until it encounters a "break" statement.
    if goAgain(firstTime):
        firstTime = False
        main(mainMenu()) # Run the main menu, then take the operation, and feed it to main().
    else: # The sleep function
