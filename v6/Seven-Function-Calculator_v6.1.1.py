"""
SEVEN-FUNCTION CALCULATOR V6.1.1
...
def get_inputs(operation) -> None:
    ...
    print(f"{inputs[0].zfill(4)} {operation} ____")
    print("       ^^^^")
    print("ENTER VALUE HERE")
...
class Calculate:
    ...
    class Arithmetic:
        ...
        def exponent(self):
            ...
            get_inputs('^')
            answer = int(inputs[0]) ** int(inputs[1])
            print(f"{str(self.inputs[0])} to the power of {str(inputs[1])} equals {str(answer)}")
    ...
    class Trig:
        ...
        def arcsin(self):
            ...
            if not answer.is_integer():
                if input("Would you like more precision? (y/n) ").strip().lower() == 'y':
...
def prompt_restart(first_time): # Perform another calculation message and surrounding computation.
    ...
    if first_time == True:
        ...
        prompt_restartStr = input("Do you want to perform another calculation? (y/n)")
        ...
        if prompt_restartStr == 'y':
            ...
            print("ERROR: Invalid input recieved when asking to restart in function prompt_restart")
            return 1
